# Absolute Docs — Ribix IDE

Planning, design, and implementation artifacts for transforming the Void fork into the Ribix IDE: an agent-first software engineering operating system.

Files in this directory define the product vision, reconstruction plan, and QA contracts.

---

## Contents

| File | Purpose |
|---|---|
| `Engineering_Plan.md` | Phase-by-phase implementation plan. Source of truth for all reconstruction work. |
| `Ribix_IDE_Overview.txt` | Product overview, feature reference, and positioning document. |
| `E2E_QA_Checklist.md` | End-to-end manual QA checklist for each major phase. |
| `text-files/Ribix_IDE_Overview.txt` | Plain-text copy of the product overview. |

---

## Current source of truth

- [`../README.md`](../README.md)
- [`../VOID_CODEBASE_GUIDE.md`](../VOID_CODEBASE_GUIDE.md)
- [`../src/vs/workbench/contrib/void/`](../src/vs/workbench/contrib/void/)

---

Confidential — Ribix Inc.

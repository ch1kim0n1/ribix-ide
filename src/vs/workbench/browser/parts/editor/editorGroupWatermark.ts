...
/* eslint-disable */ // Ribix
import { RIBIX_CTRL_K_ACTION_ID, RIBIX_CTRL_L_ACTION_ID } from '../../../contrib/ribix/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const ribixIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			// Ribix - if the workbench is empty, show open
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				ribixIconBox.appendChild(buttonContainer);

				// Open a folder
				const openFolderButton = h('button')
				openFolderButton.root.classList.add('ribix-openfolder-button')
				...
				openSSHButton.root.classList.add('ribix-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					ribixIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('ribix-link')
				...
				// show them Ribix keybindings
				const keys = this.keybindingService.lookupKeybinding(RIBIX_CTRL_L_ACTION_ID);
				const dl = append(ribixIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(RIBIX_CTRL_K_ACTION_ID);
				const dl2 = append(ribixIconBox, $('dl'));

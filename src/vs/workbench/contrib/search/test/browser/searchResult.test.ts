...
suite('SearchResult', () => {
	...
	test('replace should remove the file match', function () {
		const ribixPromise = Promise.resolve(null);
		instantiationService.stub(IReplaceService, 'replace', ribixPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange))]);
		...
		return ribixPromise.then(() => assert.ok(testObject.isEmpty()));
	});
	...
	test('replaceAll should remove all file matches', function () {
		const ribixPromise = Promise.resolve(null);
		instantiationService.stubPromise(IReplaceService, 'replace', ribixPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange)),

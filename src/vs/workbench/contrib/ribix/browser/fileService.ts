...
class FilePromptActionService extends Action2 {
	private static readonly RIBIX_COPY_FILE_PROMPT_ID = 'void.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.RIBIX_COPY_FILE_PROMPT_ID,
			title: localize2('ribixCopyPrompt', 'Ribix: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await ribixModelService.getModelSafe(uri)).model?.getLanguageId() || '',

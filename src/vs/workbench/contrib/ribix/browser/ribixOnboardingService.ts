...
import { mountRibixOnboarding } from './react/out/ribix-onboarding/index.js'
...
export class OnboardingContribution extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'workbench.contrib.ribixOnboarding';
	...
	private initialize(): void {
		...
		if (workbench) {
			...
			const onboardingContainer = h('div.ribix-onboarding-container').root;
			workbench.appendChild(onboardingContainer);
			this.instantiationService.invokeFunction((accessor: ServicesAccessor) => {
				const result = mountRibixOnboarding(onboardingContainer, accessor);

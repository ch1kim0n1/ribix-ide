...
import { RIBIX_VIEW_CONTAINER_ID, RIBIX_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { RIBIX_TOGGLE_SETTINGS_ACTION_ID } from './ribixSettingsPane.js';
...
const RIBIX_OPEN_SIDEBAR_ACTION_ID = 'void.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: RIBIX_OPEN_SIDEBAR_ACTION_ID, title: localize2('ribixOpenSidebar', 'Ribix IDE: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(RIBIX_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: RIBIX_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('ribixCmdL', 'Ribix IDE: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(RIBIX_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(RIBIX_OPEN_SIDEBAR_ACTION_ID)
		}
...
const RIBIX_CMD_SHIFT_L_ACTION_ID = 'void.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: RIBIX_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', RIBIX_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		commandService.executeCommand(RIBIX_CMD_SHIFT_L_ACTION_ID)
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(RIBIX_TOGGLE_SETTINGS_ACTION_ID)
	}
})

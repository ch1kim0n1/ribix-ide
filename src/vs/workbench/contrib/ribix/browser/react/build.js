...
function saveStylesFile() {
	setTimeout(() => {
		try {
			const pathToCssFile = findDesiredPathFromLocalPath('./src/vs/workbench/contrib/ribix/browser/react/src2/styles.css', __dirname);
...
if (isWatch) {
	...
	if (!fs.existsSync('src2')) {
		try {
			...
			execSync(
				'npx scope-tailwind ./src -o src2/ -s ribix-scope -c styles.css -p "ribix-"',
				{ stdio: 'inherit' }
			);
	...
	execSync('npx scope-tailwind ./src -o src2/ -s ribix-scope -c styles.css -p "ribix-"', { stdio: 'inherit' });

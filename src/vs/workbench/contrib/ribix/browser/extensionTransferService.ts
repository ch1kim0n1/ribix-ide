...
const extensionBlacklist = [
	...
	// ignore other AI copilots that could conflict with Ribix keybindings
	'sourcegraph.cody-ai',
	'continue.continue',
	'codeium.codeium',
...
class ExtensionTransferService extends Disposable implements IExtensionTransferService {
	...
	async deleteBlacklistExtensions(os: 'mac' | 'windows' | 'linux' | null) {
		...
		let eURI: Awaited<ReturnType<typeof fileService.resolve>>
		try {
			eURI = await fileService.resolve(extensionsURI)
		} catch {
			// Extensions folder doesn't exist yet on fresh installs — nothing to clean up
			return
		}
	...
	if (os === 'mac') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Ribix', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Ribix', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.ribix-ide', 'extensions'),
				isExtensions: true,
			}]
	...
	if (os === 'linux') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Ribix', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Ribix', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.ribix-ide', 'extensions'),
				isExtensions: true,
	...
	if (os === 'windows') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Ribix', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Ribix', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), userprofile, '.ribix-ide', 'extensions'),
				isExtensions: true,
			}]
		} else if (fromEditor === 'Cursor') {
			return [{

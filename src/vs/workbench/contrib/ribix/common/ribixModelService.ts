...
export interface IRibixModelService {
	readonly _serviceBrand: undefined;
	initializeModel(uri: URI): Promise<void>;
	getModel(uri: URI): RibixModelType;
	getModelFromFsPath(fsPath: string): RibixModelType;
	getModelSafe(uri: URI): Promise<RibixModelType>;
	saveModel(uri: URI): Promise<void>;

}

export const IRibixModelService = createDecorator<IRibixModelService>('ribixRibixModelService');

class RibixModelService extends Disposable implements IRibixModelService {
	_serviceBrand: undefined;
	static readonly ID = 'ribixRibixModelService';
	...
	getModelSafe = async (uri: URI): Promise<RibixModelType> => {
...
registerSingleton(IRibixModelService, RibixModelService, InstantiationType.Eager);

...
export class DebugHoverWidget implements IContentWidget {
	...
	private layoutTree(): void {
		...
		let maxHeightToAribixCursorOverlay = Infinity;
		if (this.showAtPosition) {
			...
			if (containerTop < hoveredCharTop) {
				maxHeightToAribixCursorOverlay = hoveredCharTop + editorTop - 22; // 22 is monaco top padding https://github.com/microsoft/vscode/blob/a1df2d7319382d42f66ad7f411af01e4cc49c80a/src/vs/editor/browser/viewParts/contentWidgets/contentWidgets.ts#L364
			}
		}
		const treeHeight = Math.min(Math.max(266, this.editor.getLayoutInfo().height * 0.55), this.tree.contentHeight + scrollBarHeight, maxHeightToAribixCursorOverlay);
